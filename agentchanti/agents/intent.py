...
class IntentAgent(Agent):
        ...
        if _is_blank_project:
            ...
            try:
                ...
                if _tech_resp.upper() != "NONE" and _tech_resp:
                    ...
                    if _techs:
                        accumulated_context += (
                            "PACKAGE VERSION STRATEGY (CRITICAL for new projects):\n"
                            "1. If the user specifies an EXACT version in their "
                            "prompt (e.g. 'react 18', 'tailwind 3'), pin THAT "
                            "version and all companion packages to compatible "
                            "versions from the guides above.\n"
                            "2. If NO version is specified: do NOT pin exact "
                            "versions. Use bare package names in install "
                            "commands (e.g. 'npm install react react-dom' not "
                            "'npm install react@19.0.0 react-dom@19.0.0'). "
                            "Let the scaffold and npm resolve compatible "
                            "versions automatically.\n"
                            "3. NEVER mix pinned old versions with a @latest "
                            "scaffold. If you use 'npm create vite@latest' "
                            "(which installs the newest Vite), companion "
                            "packages like @vitejs/plugin-react MUST also be "
                            "unpinned so npm picks a compatible release.\n"
                            "4. Config file patterns (tailwind.config, "
                            "postcss.config, vite.config) MUST match the "
                            "major version that gets installed. Use the "
                            "patterns from the setup guides above — they "
                            "describe the current version's format.\n"
                            "WHY: Pinning old companion packages (e.g. "
                            "@vitejs/plugin-react@4.0.0) alongside a latest "
                            "scaffold (vite@8) causes ERESOLVE peer conflicts "
                            "and blocks installation entirely.\n\n"
                        )
    ...
    _SPEC_FEATURE = (
        ...
        "Acceptance: <a concrete, runnable check proving the capability works: the "
        "exact command to run it AND the observable outcome the user must see or "
        "interact with. For a visual/interactive app, say what appears on screen and "
        "how the user controls it (e.g. 'python -m app.main opens a window rendering "
        "the room; arrow keys/mouse move the camera'). An import succeeding or a unit "
        "test passing is NOT acceptance for a user-facing app.>\n"
    ...
    _SPEC_MODIFY = (
        ...
        "Acceptance: <the observable outcome after the change AND how to run/verify "
        "it; for a user-facing change, an import or unit test passing is NOT "
        "acceptance — describe what the user sees or does differently>\n"
    ...
    def _build_prompt(self, raw_task: str, accumulated_context: str, conclude: bool = False) -> str:
        ...
        prompt += (
            ...
            "  For a visual or interactive app (game, GUI, 3D/canvas, animation,\n"
            "  simulation): commit to a CONCRETE rendering/runtime backend (e.g.\n"
            "  pygame, tkinter, a web canvas) and state in Acceptance what the user\n"
            "  SEES and how they interact. Do NOT deliver backend-agnostic stubs that\n"
            "  only satisfy an import — a module that computes geometry but never\n"
            "  opens a window or draws anything does not meet the request.\n\n"
            "MODIFY:\n"
