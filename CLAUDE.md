...
The silences carry as much of the design as the finding. A tautology is deliberately not a pin — `assertIn(game.state, (PLAYING, WIN, GAME_OVER))` admits the terminal states it was meant to exclude, and a real run wrote exactly that inside a 2100-iteration loop. Silent when the test pins the state (in the loop *or* after it), breaks out, reacts to the state at all — `if game.state is GAME_OVER: game.restart()` kept one suite live for 2000 of 2000 frames across six restarts — or deliberately extends the run's lifetime (`game.spawn_protection_timer = 1_000_000.0` before the loop). Each of those silences was added after the check reported a suite that a runtime probe then proved was live for every frame; a check that cries wolf is one that gets ignored.
