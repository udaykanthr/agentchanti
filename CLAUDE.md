...
Proved structurally, from two halves that must both hold: the advance method early-returns on a state guard (`guarded_advance_methods`, which scans the whole guard prologue — a real `update()` put `if not math.isfinite(dt): raise` first), and a long `range(N)` loop calls it without ever pinning the state to a live value. Both come from the project's own source, so nothing is hardcoded about what a "game" or a "frame" is. A tautology is deliberately not a pin: `assertIn(game.state, (PLAYING, WIN, GAME_OVER))` admits the terminal states it was meant to exclude, and a real run wrote exactly that inside a 2100-iteration loop. Silent when the test pins the state (in the loop *or* after it), breaks out on its own, or deliberately extends the run's lifetime (`game.spawn_protection_timer = 1_000_000.0` before the loop) — two runs did that and were live for every frame, and accusing them is how a check gets ignored.
