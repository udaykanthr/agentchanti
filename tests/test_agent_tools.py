...
class TestWriteScope(AgentToolsTestCase):
    """A stage that authors one file must not overwrite other deliverables.

    Observed: the property check rewrote the plan's own tests/test_game.py,
    discarding the 600-frame adversarial suite written and verified in an
    earlier wave, then edited main.py — and still never produced
    test_properties.py, so the stage reported "skipped" while its edits
    stayed on disk and reached the final commit.
    """

    def setUp(self):
        super().setUp()
        self.scoped = AgentTools(project_root=self.root,
                                 write_scope=["test_properties.py", "main.py"])
        self._write("tests/test_game.py", "# verified deliverable\n")
        self._write("main.py", "X = 1\n")

    def _scoped(self, name, **args):
        return self.scoped.execute(ToolCall(name=name, arguments=args))

    def test_writes_inside_the_scope_succeed(self):
        out = self._scoped("write_file", path="test_properties.py",
                           content="import unittest\n")
        self.assertTrue(out.startswith("OK"), out)

    def test_the_source_under_test_stays_writable(self):
        """The prompt tells it to fix a real violation in the source."""
        out = self._scoped("write_file", path="main.py", content="X = 2\n")
        self.assertTrue(out.startswith("OK"), out)

    def test_writing_another_steps_test_file_is_refused(self):
        out = self._scoped("write_file", path="tests/test_game.py",
                           content="wiped")
        self.assertTrue(out.startswith("ERROR"), out)
        self.assertIn("test_properties.py", out)
        with open(os.path.join(self.root, "tests/test_game.py"),
                  encoding="utf-8") as fh:
            self.assertEqual(fh.read(), "# verified deliverable\n")

    def test_editing_outside_the_scope_is_refused(self):
        out = self._scoped("edit_file", path="tests/test_game.py",
                           old_text="verified", new_text="x")
        self.assertTrue(out.startswith("ERROR"), out)
        with open(os.path.join(self.root, "tests/test_game.py"),
                  encoding="utf-8") as fh:
            self.assertIn("verified", fh.read())

    def test_reads_are_never_restricted(self):
        out = self._scoped("read_file", path="tests/test_game.py")
        self.assertIn("verified deliverable", out)

    def test_leading_dot_slash_still_matches_the_scope(self):
        out = self._scoped("write_file", path="./test_properties.py",
                           content="import unittest\n")
        self.assertTrue(out.startswith("OK"), out)

    def test_backslash_paths_still_match_the_scope(self):
        scoped = AgentTools(project_root=self.root,
                            write_scope=["tests/test_properties.py"])
        out = scoped.execute(ToolCall(
            name="write_file",
            arguments={"path": r"tests\test_properties.py",
                       "content": "import unittest\n"}))
        self.assertTrue(out.startswith("OK"), out)

    def test_no_scope_means_unrestricted(self):
        """Every ordinary step must keep full write access."""
        out = self._call("write_file", path="anything/at/all.py",
                         content="a = 1\n")
        self.assertTrue(out.startswith("OK"), out)
