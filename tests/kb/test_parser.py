"""
Unit tests for agentchanti.kb.local.parser
...
class TestDetectLanguage:
    def test_python(self):
        from agentchanti.kb.local.parser import detect_language
...
class TestPythonParsing:
    ...
    def test_returns_parsed_file(self, tmp_py):
        from agentchanti.kb.local.parser import parse_file
...
class TestComputeFileHash:
    def test_consistent_hash(self, tmp_path):
        from agentchanti.kb.local.parser import compute_file_hash
        p = tmp_path / "foo.py"
        p.write_text("x = 1\n")
