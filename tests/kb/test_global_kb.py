...
class TestSeeder(unittest.TestCase):
    ...
    def test_seed_no_embed(self):
        ...
        self.assertEqual(summary["docs_seeded"], 11)  # 3+2+3+3
        ...
        expected_per_lang = {
            "python": 5, "javascript": 24,  # 5 base + 2 npm + 4 react-testing + 1 waitFor-assumed-content + 1 nested-router + 1 assumed-aria + 1 queryByRole-null + 1 rerender-memoryrouter + 2 vitest + 1 no-suite + 1 vite-resolve + 1 component-type-invalid + 1 suspense-fallback-sync-query
