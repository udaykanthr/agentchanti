...
class TestSeeder(unittest.TestCase):
    ...
    def test_seed_no_embed(self):
        ...
        self.assertEqual(summary["errors_seeded"], 54)
        self.assertGreaterEqual(summary["content_fixes_seeded"], 1)
        self.assertEqual(summary["docs_seeded"], 10)  # 3+2+2+3
        ...
        expected_per_lang = {
            "python": 5, "javascript": 21,  # 5 base + 2 npm + 4 react-testing + 1 assumed-aria + 1 queryByRole-null + 1 rerender-memoryrouter + 2 vitest + 1 no-suite + 1 vite-resolve + 1 component-type-invalid
